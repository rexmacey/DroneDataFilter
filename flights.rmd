...
print(paste("% of observations used in these flight:", round(100 * 2 * nrow(flights) / result$nObs, 0)))
...
```
```{r FlightsByBee}
ggplot(flights, aes(x = Departure, y = TAG)) + 
  geom_point(aes(colour = DepHive, size = as.numeric(Duration)))
```
...
ggplot(flights, aes(x = as.numeric(Age), y = Duration)) + geom_point() +
